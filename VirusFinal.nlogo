...
[ percent_positive         ;; what percent of the population is covid-positive
  ...
  vaccine_effectiveness_reduction ] ;; reduction in infection chance due to vaccination
...
let num_vaccinated round((percent_vaccinated / 100) * population)
create-turtles population
  [ set shape "person" 
    ...
    set vaccinated? false ] ;; Ensure all turtles start as not vaccinated
ask n-of 10 turtles [ become_infected ]
ask n-of num_vaccinated turtles [ vaccinate ]
end
...
ask turtles
[ set color ifelse-value covid_positive? [ red ] [ ifelse-value is_immune? [ grey ] [ ifelse-value vaccinated? [ blue ] [ green ] ] ] ]
end
...
to transmit_virus
ask turtles [ if mask-effectiveness < 0.1 [set mask-effectiveness 0.1] ]
  
ask other turtles-here with [ not covid_positive? and not is_immune? ]
[
	let effective_transmission_modifier transmission_modifier
  if wearing_mask? [
    set effective_transmission_modifier effective_transmission_modifier * mask-effectiveness
  ]
  if vaccinated? [
    set effective_transmission_modifier effective_transmission_modifier * vaccine_effectiveness_reduction
  ]
    
  if random-float 100 < (infectiousness * effective_transmission_modifier) / (ifelse-value wearing_mask? [mask-effectiveness] [1]) [
    become_infected
  ]
]
end
...
to-report transmission_modifier
let modifier 1
